...
const AIM_ROTATION_OFFSET := PI / 2
...
func _process(delta):
	if active or findClosestPoint():
		show()
		look_at(closestPoint)
		rotation += AIM_ROTATION_OFFSET
	else:
		hide()
...
func doneGrapple():
	...
	get_parent().coyote_time = .3
	get_parent().velocity.y = -30
